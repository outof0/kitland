...
} from "../tools/regex-tester.worker-protocol";
import { useEffect, useMemo, useRef, useState } from "react";

const TEST_DEBOUNCE_MS = 120;
const UNAVAILABLE_ERROR = {
  code: "WORKER_UNAVAILABLE",
  message: "The local regex tester is unavailable. Reopen the extension and try again.",
} as const;
...
/**
 * Browser-extension host hook for the shared Regex Tester. User-provided
 * patterns run in a packaged Web Worker so a catastrophic pattern cannot freeze
 * the extension page; the synchronous core `testRegex` is only the explicit
