...
const archivePath = resolve(
  artifactDirectory,
  `kitland-browser-extension-v${packageJson.version}.zip`,
);
...
console.log(
  `Created ${basename(archivePath)} (${formatBytes(archive.byteLength)}, ${files.length} files)`,
);
...
function createZip(entries) {
  ...
  for (const entry of entries) {
    ...
    localHeader.writeUInt16LE(33, 12); // 1980-01-01, deterministic package timestamp.
