...
- Astro statically emits an HTML document for the landing page, every
  `/explore/<slug>` `available` catalog tool, and the real 404 page. Each tool
...
- `/explore` is an emitted, indexable catalog and implementation-status
...
- `@astrojs/sitemap` emits `sitemap-index.xml` from the generated routes, so
  the sitemap does not duplicate catalog slugs by hand.
...
- `BaseLayout.astro` owns title, description, canonical URL, Open Graph,
  ...
  `WebApplication`, `BreadcrumbList`, and `FAQPage` schemas from catalog-owned
  content.

## Indexing requirements

- Every indexable route must have unique, human-written title/description and
  substantive static tool guidance. Do not add a catalog entry until its SEO
...
remain in the artifact, confirms `/explore` is a static sitemap-listed catalog
