...
| `packages/ui/tokens.css` (via `@kitland/ui/theme.css`) | Kitland semantic colors, typography, elevation, and theme values — single source for web, extension, and VS Code webview | Page-specific pixel values or component-specific state logic                                              |
...
| Share privacy     | Create a URL fragment only after explicit activation (Share click). The disclosure must say that the link contains the current input and should not be used for secrets (`Share links include the current input. Don’t share secrets.`). Never put tool input in a query string, upload it, or persist it by default. Strip the current query string from the shared URL. Malformed or oversized fragments restore nothing and must not wipe live editor state the user already has. Clipboard failure is actionable and must not invent a second payload channel. Base64 and JSON Formatter implement this path; only hosts that declare `share-link` may offer it. |
...
## Adoption sequence for the 65-tool rollout
