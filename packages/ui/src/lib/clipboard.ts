...
function isTextFile(file: File, accept: string): boolean {
  ...
  // Allow exact MIME matches declared in accept (e.g., application/json, application/x-yaml).
  const acceptTokens = accept
    .split(",")
    .map((t) => t.trim().toLowerCase())
    .filter(Boolean);
  if (file.type && acceptTokens.includes(file.type.toLowerCase())) return true;
  // Extension allowlist = built-in text extensions + every .ext token from accept
