...
export type Base64WorkerResponse = {
  ...
  /** Number of CRLF, CR, or LF-delimited input lines, measured off the main thread. */
  inputLineCount: number;
};

export function countBase64InputLines(value: string): number {
  let lines = 1;
...
export function isBase64WorkerResponse(value: unknown): value is Base64WorkerResponse {
  ...
  return (
    ...
    isByteLength(value.outputByteLength) &&
    isLineCount(value.inputLineCount)
  );
}
