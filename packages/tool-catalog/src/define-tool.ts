...
export function defineTool<const T extends ToolDefinition>(definition: T): T {
  ...
  if (definition.releaseStage === "release-ready") {
    const unresolvedPlatform = TOOL_PLATFORM_IDS.find(
      (platform) => definition.platforms[platform].status === "planned",
    );
    if (definition.status !== "available" || unresolvedPlatform) {
      throw new Error(
        `Release-ready tool "${definition.slug}" must be web-available with no planned platform contracts.`,
      );
    }
  }
  ...
  Object.freeze(definition.platforms);
